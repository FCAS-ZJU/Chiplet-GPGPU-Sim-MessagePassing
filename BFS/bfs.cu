...
//    CUT_DEVICE_INIT();
	...
	int num_of_blocks = 1;
	int num_of_threads_per_block = no_of_nodes/4;
	...
	/*if(i>2*no_of_nodes/4 || i<=no_of_nodes/4){                 ////////
...
/*************/
//打开文件并读取
	std::fstream file("/home/ly/桌面/connect/connect_D.txt",std::ios::in);
	int mask[10000];
	int i;
	for(i =0 ;!file.eof();i++){
	file >> mask[i];
	if(mask[i]<4096 && mask[i] >0){	
	
        std::cout<<mask[i]<<" ";
	std::cout<<mask[i]<<"\n";
	if(h_graph_visited[mask[i]] == false){
	h_graph_mask[mask[i]] = true;
	h_graph_visited[mask[i]] = true;
//	if(h_graph_visited[mask[j]] == false)
		h_cost[mask[i]] += 1;}}
	}
...
//清空通讯文件
	...
	file.close();
	/*
std::fstream connectfile("/home/ly/桌面/connect/connect_A.txt",std::ios::out);
connectfile.close();*/
...
cudaMemcpy( h_graph_visited, d_graph_visited, sizeof(bool)*no_of_nodes, cudaMemcpyDeviceToHost) ;
	...
	cudaMemcpy(h_graph_mask,  d_graph_mask, sizeof(bool)*no_of_nodes, cudaMemcpyDeviceToHost);
        k++;	
	cudaMemcpy( h_cost, d_cost, sizeof(int)*no_of_nodes, cudaMemcpyDeviceToHost);
	FILE *fpo1 = fopen("result.txt","w");
	for(int i=0;i<no_of_nodes;i++)
	fprintf(fpo1,"%d) cost:%d\n",i,h_cost[i]);
